...
class Extractor(Parser):
    ...
    def generate(self):
        ...
        for offering in tqdm(self.data.itertuples(index=False), total=len(self.data.index)):
            ...
            if instructor == '': instructor = "UNKNOWN"
            # instructor = 'TEST'
            sections = 1
            readj = lambda new, prev, count: (new*(dist['Students']) + prev*(old["Students"])) / (count)
            dist = {'A': offering.A, 'AB': offering.AB, 'B': offering.B, 'BC': offering.BC, 'C': offering.C, 'D': offering.D, 'F': offering.F, 'GPA': offering.GPA, 'Students': int(offering.Students), 'Sections': sections}
